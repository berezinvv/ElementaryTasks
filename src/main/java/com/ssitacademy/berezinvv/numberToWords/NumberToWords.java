...
public class NumberToWords {
    ...
    private static String converNumberToWords(BigDecimal number) {
        ...
        for (String str : list) {
            ...
            String srt100 = str.substring(0, 1);
            String str10 = str.substring(1, 2);
            int num10_19 = Integer.parseInt(str.substring(1, 3));
            int num1 = Integer.parseInt(str.substring(2, 3));

            if (!srt100.equals("0")) {
                res.append(textDigital.get(srt100 + "00") + " ");
            }
            if (num10_19 >= 10 && num10_19 <= 19) {
                res.append(textDigital.get(String.valueOf(num10_19)) + " ");
            } else {
                if (!str10.equals("0")) {
                    res.append(textDigital.get(str10 + "0") + " ");
                }
                if (num1 != 0) {
                    res.append(textDigital.get(String.valueOf(((num1 == 1 || num1 == 2) && level % 2 != 0) ? -num1 : num1)) + " ");
                }
            }
            if (level >= 1) {
                if (num1 == 0 || num1 >= 5) {
                    res.append(textLevel.get(String.valueOf(level) + "5") + " ");
                } else if (num1 >= 2 || num1 <= 4) {
                    res.append(textLevel.get(String.valueOf(level) + "2") + " ");
                } else {
                    res.append(textLevel.get(String.valueOf(level) + num1) + " ");
                }
            }
            level--;
        }
